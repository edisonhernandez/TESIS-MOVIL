...
let LoginPage = class LoginPage {
    constructor(nativeStorage, loadingController, fb, _route, _router, _usuarioService, toastController, alertController) {
    ...
    login() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            this._usuarioService.login(this.usuarioLogin).subscribe(response => {
    ...
    loginFacebook() {
        const rs = this.fb.login(['public_profile', 'email'])
            .then((res) => this.presentAlert('success', res))
                ...
                this.fb.api("/me?fields=name,email", permissions)
                    ...
                    this._usuarioService.validUser(us).subscribe(response => {
                        ...
                        if (response.n == '1') { //no registrado -- empieza proceso automatico de registro
                            ...
                            this._usuarioService.loginFB(this.usuarioLogin2, 'true').subscribe(response => {
                                this.token = response.token;
                                console.log(this.token);
                                if (this.token.length <= 0) {
                                    this.presentToast('No se pudo guardar el token');
